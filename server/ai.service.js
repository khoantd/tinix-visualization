...
/**
 * Gọi chat completion qua provider đã cấu hình (OpenRouter hoặc LiteLLM)
 */
async function callChatCompletion(prompt, providerId) {
  const resolved = assertProviderConfigured(providerId);
  const config = getProviderConfig(resolved);

  console.log(`[AI] Using provider: ${resolved} | model: ${config.model}`);
 ...
 */
async function analyzeDatasetSchema(sampleData, providerId) {
...
Yêu cầu: "name" phải trùng khớp 100% với key trong dữ liệu JSON mẫu. Chỉ trả về JSON thuần.`;
  ...
  try {
    const response = await callChatCompletion(prompt, providerId);
 ...
 */
async function suggestCharts(confirmedSchema, providerId) {
  assertProviderConfigured(providerId);
